<?php
require 'require.php';
$data = JsonHelper::read('BigData');
$status = isset($_GET['status']) ? $_GET['status'] : '';
$filter = [1, 2, 8, 30, 36, 73, 95, 127];
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                $('#filter').on('change', function () {
                    if($('#filter').val() == 1) {
                        window.location.href = '/big.php?status=1';
                    }else if($('#filter').val() === '0') {
                        window.location.href = '/big.php?status=0';
                    }else{
                        window.location.href = '/big.php';
                    }
                });
                $('.edit').on('click', function() {
                    ...
                    $.ajax({
                        url:'/save.php',
                        data:{'type':'BigData','key':parseInt(key), 'name':data},
                        type:'POST',
